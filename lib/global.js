...
function GlobalContextProviderInner(props) {
  const {
    post,
    children,
    siteInfo,
  ...
  // 登录验证相关
  const { isLoaded, isSignedIn, user } = authState
...
function ClerkEnabledGlobalContextProvider(props) {
  const { isLoaded, isSignedIn, user } = useUser()
  return (
    <GlobalContextProviderInner
      {...props}
      authState={{ isLoaded, isSignedIn, user }}
    />
  )
}
...
export function GlobalContextProvider(props) {
  ...
  if (enableClerk) {
    return <ClerkEnabledGlobalContextProvider {...props} />
  }

  return (
    <GlobalContextProviderInner
      {...props}
      authState={{ isLoaded: true, isSignedIn: false, user: null }}
    />
  )
}
