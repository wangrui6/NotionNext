...
// Build-time pages often request the same Notion payload concurrently.
// Deduplicate in-flight work by cache key so we only hit Notion once.
...
export function getApi() {
  if (BLOG.REDIS_URL) {
    return RedisCache
  } else if (isEnabled(process.env.ENABLE_FILE_CACHE) || useFileCacheDuringBuild) {
    return FileCache
  } else {
    return MemoryCache
  }
