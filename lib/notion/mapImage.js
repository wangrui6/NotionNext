...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    ...
    if (
      ...
      ret = `${ret.trim()}${separator}t=${block?.id || '0'}`
    }
  }
...
function normalizeImageInput(img) {
  if (typeof img === 'string') {
    return img
  }

  if (Array.isArray(img)) {
    const candidate = img.find(item => typeof item === 'string')
    return candidate || null
  }

  if (img && typeof img === 'object') {
    const candidate =
      img.url || img.src || img.value || img.path || img.display_source
