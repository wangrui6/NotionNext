...
function convertNotionBlocksToPost(id, blockMap, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    const rawBlock = clonePageBlock?.block[blockId]

    if (rawBlock?.value?.value?.id) {
      clonePageBlock.block[blockId] = { value: rawBlock.value.value }
    }

    const b = clonePageBlock?.block[blockId]

    if (!b?.value?.id && b?.value?.role !== undefined) {
      continue
    }
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const batchKey = crypto
      ...
      .update(batch.join(','))
      .digest('hex')
    const pageChunk = await getOrSetDataWithCache(
      `missing_blocks_${batchKey}`,
      async currentBatch => {
        console.log('[API-->>请求] Fetching missing blocks', currentBatch, ids.length)
        const start = new Date().getTime()
        const currentChunk = await notionAPI.getBlocks(currentBatch)
        const end = new Date().getTime()
        console.log(
          `[API<<--响应] 耗时:${end - start}ms Fetching missing blocks count:${ids.length} `
        )
        console.log('[API<<--响应]')
        return currentChunk?.recordMap?.block || {}
      },
      batch
    )
