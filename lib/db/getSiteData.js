...
export async function getGlobalData({
  ...
  return handleDataBeforeReturn(deepClone(data), pageId)
}
...
function normalizeSiteDataShape(db, pageId) {
  const empty = EmptyData(pageId)
  const siteData = { ...empty, ...(db || {}) }

  siteData.NOTION_CONFIG = siteData.NOTION_CONFIG || {}
  siteData.siteInfo = siteData.siteInfo || empty.siteInfo
  siteData.notice = siteData.notice ?? null
  siteData.allPages = Array.isArray(siteData.allPages)
    ? siteData.allPages
    : empty.allPages
  siteData.allNavPages = Array.isArray(siteData.allNavPages)
    ? siteData.allNavPages
    : []
  siteData.collection = Array.isArray(siteData.collection)
    ? siteData.collection
    : []
  siteData.viewIds = Array.isArray(siteData.viewIds) ? siteData.viewIds : []
  siteData.pageIds = Array.isArray(siteData.pageIds) ? siteData.pageIds : []
  siteData.tagOptions = Array.isArray(siteData.tagOptions)
    ? siteData.tagOptions
    : []
  siteData.categoryOptions = Array.isArray(siteData.categoryOptions)
    ? siteData.categoryOptions
    : []
  siteData.customNav = Array.isArray(siteData.customNav)
    ? siteData.customNav
    : []
  siteData.customMenu = Array.isArray(siteData.customMenu)
    ? siteData.customMenu
    : []
  siteData.latestPosts = Array.isArray(siteData.latestPosts)
    ? siteData.latestPosts
    : []
  siteData.collectionQuery = siteData.collectionQuery || {}
  siteData.collectionView = siteData.collectionView || {}
  siteData.block = siteData.block || {}
  siteData.schema = siteData.schema || {}
  siteData.rawMetadata = siteData.rawMetadata || {}
  siteData.collectionId = siteData.collectionId || null

  if (typeof siteData.postCount !== 'number') {
    siteData.postCount = Array.isArray(siteData.allPages)
...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const value = normalizePageBlock(block[id] || fetchedBlocks[id])
    if (!value) {
      continue
    }
    if (value.parent_id !== collectionId) {
      continue
    }
    const properties =
 ...
 * @param {*} db
 */
function handleDataBeforeReturn(db, pageId) {
  db = normalizeSiteDataShape(db, pageId)
...
function normalizePageRecord(page) {
  page.pageCoverThumbnail =
    typeof page.pageCoverThumbnail === 'string'
      ? page.pageCoverThumbnail
      : page.pageCoverThumbnail || ''
  page.pageCover =
    typeof page.pageCover === 'string' ? page.pageCover : page.pageCover || ''
  page.pageIcon =
    typeof page.pageIcon === 'string' ? page.pageIcon : page.pageIcon || ''
  page.summary =
    typeof page.summary === 'string' ? page.summary : page.summary || ''
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const pageCover = collection?.cover
    ? mapImgUrl(collection?.cover, collection, 'collection') || defaultPageCover
    : defaultPageCover

  // 用户头像压缩一下
  let icon = compressImage(
    collection?.icon
      ? mapImgUrl(collection?.icon, collection, 'collection') || defaultIcon
      : defaultIcon
  )
  // 站点网址
